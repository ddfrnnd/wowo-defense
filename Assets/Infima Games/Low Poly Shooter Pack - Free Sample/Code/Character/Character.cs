...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		protected override void Start()
		{
			...
			// Configure URP Camera Stacking
			if (cameraWorld != null)
			{
				var baseCameraData = cameraWorld.GetComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
				if (baseCameraData == null)
				{
					baseCameraData = cameraWorld.gameObject.AddComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
				}

				Transform depthCamTransform = cameraWorld.transform.Find("Camera Depth");
				if (depthCamTransform != null)
				{
					Camera depthCamera = depthCamTransform.GetComponent<Camera>();
					if (depthCamera != null)
					{
						var overlayCameraData = depthCamera.GetComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
						if (overlayCameraData == null)
						{
							overlayCameraData = depthCamera.gameObject.AddComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
						}

						// Set World camera as Base
						baseCameraData.renderType = UnityEngine.Rendering.Universal.CameraRenderType.Base;

						// Set Weapon camera as Overlay
						overlayCameraData.renderType = UnityEngine.Rendering.Universal.CameraRenderType.Overlay;

						// Add Weapon camera to World camera stack
						baseCameraData.cameraStack.Clear();
						baseCameraData.cameraStack.Add(depthCamera);
						
						Debug.Log("[URP Camera Stack] Successfully stacked Camera Depth onto Camera.");
					}
				}
			}
		}
		...
		protected override void Update()
		{
			...
			// Numeric keys weapon switching
			for (int i = 0; i < 9; i++)
			{
				if (Input.GetKeyDown(KeyCode.Alpha1 + i))
				{
					if (CanChangeWeapon() && inventory.GetEquippedIndex() != i)
					{
						StartCoroutine(nameof(Equip), i);
					}
				}
			}
		...
		public override bool IsCursorLocked()
		{
			...
			return cursorLocked || Application.isMobilePlatform || mobileAxisLook != Vector2.zero || mobileAxisMovement != Vector2.zero;
		}
		...
		{
			if (Time.timeScale == 0f) return Vector2.zero;
			return axisMovement + mobileAxisMovement;
		}
		public override Vector2 GetInputLook()
		{
			if (Time.timeScale == 0f) return Vector2.zero;
			return axisLook + mobileAxisLook;
		}
		...
		private void UpdateAnimator()
		{
			...
			characterAnimator.SetFloat(HashMovement, Mathf.Clamp01(Mathf.Abs(inputMovement.x) + Mathf.Abs(inputMovement.y)), dampTimeLocomotion, Time.deltaTime);
		...
		private bool CanPlayAnimationReload()
		{
			...
			//Block if weapon is already full
			if (equippedWeapon != null && equippedWeapon.IsFull())
				return false;

			//Block if reserve ammo is empty
		...
		private bool CanRun()
		{
			...
			Vector2 inputMovement = GetInputMovement();
			if (inputMovement.y <= 0 || Math.Abs(Mathf.Abs(inputMovement.x) - 1) < 0.01f)
				return false;
		...
		private Vector2 mobileAxisMovement;
		private Vector2 mobileAxisLook;

		public override void SetMobileMovement(Vector2 input)
		{
			mobileAxisMovement = input;
		}

		public override void SetMobileLook(Vector2 input)
		{
			mobileAxisLook = input;
		}

		public override void SetMobileRun(bool isRunning)
		{
			holdingButtonRun = isRunning;
		}

		public override void SetMobileAim(bool isAiming)
		{
			holdingButtonAim = isAiming;
		}

		public override void MobileFire()
		{
			if (CanPlayAnimationFire() && equippedWeapon != null)
			{
				if (equippedWeapon.HasAmmunition())
				{
					if (Time.time - lastShotTime > 60.0f / equippedWeapon.GetRateOfFire())
		...
		public override void MobileReload()
		{
			if (CanPlayAnimationReload())
			{
				PlayReloadAnimation();
		...
		public override void MobileWeaponSwitch(int index)
		{
		...
		public override void MobileJump()
		{
			var movement = GetComponent<Movement>();
			if (movement != null)
			{
				movement.TriggerMobileJump();
			}
		}
