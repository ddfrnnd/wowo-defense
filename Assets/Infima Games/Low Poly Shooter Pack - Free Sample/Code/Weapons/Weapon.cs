...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public class Weapon : WeaponBehaviour
    {
        ...
        /// Ammunition Reserve.
        /// </summary>
        [Header("Ammunition Reserve")]
        [SerializeField]
        private int ammunitionReserve = 300;
        ...
        public override void FillAmmunition(int amount)
        {
            if (amount != 0)
            {
                // Update reserve pool (e.g. from pickup)
                ammunitionReserve = Mathf.Clamp(ammunitionReserve + amount, 0, 9999);
            }
            else
            {
                // Reload from reserve
                int needed = GetAmmunitionTotal() - ammunitionCurrent;
                if (needed > 0)
                {
                    int toFill = Mathf.Min(needed, ammunitionReserve);
                    ammunitionCurrent += toFill;
                    ammunitionReserve -= toFill;
                }
            }
        }
